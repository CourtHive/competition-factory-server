import { isEvaluableScope, isTargetInScope, isTournamentWide, isWithinWindow, requiredTargetKeys } from './grantScope';
...
describe('grantScope', () => {
  describe('empty scope is tournament-wide', () => {
    it('treats {} and undefined as unrestricted — the pre-existing behavior', () => {
      ...
      expect(isTargetInScope({}, { matchUpIds: 'm1' })).toBe(true);
  ...
  describe('the court case — a permission a global boolean cannot express', () => {
    ...
    it('permits a matchUp on the granted court', () => {
      expect(isTargetInScope(courtSeven, { courtIds: 'court-7' })).toBe(true);
    });

    it('refuses the final on Centre', () => {
      expect(isTargetInScope(courtSeven, { courtIds: 'centre' })).toBe(false);
    });
    ...
    it('refuses a matchUp that cannot answer the dimension', () => {
      expect(isTargetInScope(courtSeven, { courtIds: undefined })).toBe(false);
  ...
  describe('multiple dimensions', () => {
    it('requires every declared dimension to match', () => {
      ...
      expect(isTargetInScope(scope, { courtIds: 'c1', scheduledDates: '2026-08-24' })).toBe(true);
      expect(isTargetInScope(scope, { courtIds: 'c1', scheduledDates: '2026-08-25' })).toBe(false);
    });

    it('matches any value within a single dimension', () => {
      const scope = { courtIds: ['c1', 'c2'] };
      expect(isTargetInScope(scope, { courtIds: 'c2' })).toBe(true);
      expect(isTargetInScope(scope, { courtIds: 'c3' })).toBe(false);
    });

    it('ignores a dimension declared with an empty list', () => {
      expect(isTargetInScope({ courtIds: [] }, { courtIds: undefined })).toBe(true);
    });
  });
  ...
  describe('unknown keys fail closed (A3)', () => {
    it('refuses a scope it cannot evaluate rather than waving it through', () => {
      ...
      expect(isTargetInScope(scope, { courtIds: 'court-7' })).toBe(false);
    });
  ...
  describe('requiredTargetKeys drives lazy resolution', () => {
    it('reports nothing for a tournament-wide grant, so no record walk is paid for', () => {
      expect(requiredTargetKeys({})).toEqual([]);
      expect(requiredTargetKeys(undefined)).toEqual([]);
    });

    it('reports only dimensions that actually constrain', () => {
      expect(requiredTargetKeys({ courtIds: ['c1'], matchUpIds: [] })).toEqual(['courtIds']);
    });
  });
});
