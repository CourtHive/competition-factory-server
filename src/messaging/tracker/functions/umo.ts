...
umo.stateObject = ({ index, object, parent_object, child, format, common = umo.common() }) => {
  ...
  so.change.pointScore = (value) => {
    ...
    if (object == 'Game') {
      ...
      const valid_values: any = [].concat(...Object.keys(progression).map((key) => progression[key]));
...
umo.common = () => {
  ...
  const accessors = {
    ...
    players(index?: number) {
      ...
      if (index === undefined || isNaN(index) || index < 0 || index > 3) return false;
  ...
  const pub = {
    ...
    stats: {
      ...
      counters(set_filter) {
        ...
        if (!stat_points) {
          pub.live_stats = true;
          let episodes: any = pub.history.filter((episode: any) => episode.action === 'addPoint');
    ...
    removeStatPoint(episode) {
      ...
      const points = pub.history.filter((episode: any) => episode.action == 'addPoint');
  ...
  function calculatedStats(stats) {
    ...
    const reduceComponents = (components, teams, team) => {
      ...
      return [0].concat(...values).reduce((a, b) => a + b);
    };
    ...
    const stat_calcs = {
      ...
      aggressiveMargin(stat_obj, teams, team) {
        const { numerator = 0, denominator = 0 } = numeratorDenominator(stat_obj, teams, team);
        ...
        const total_points = [0].concat(...point_counts).reduce((a, b) => a + b);
    ...
    return Object.keys(calculated_stats)
      .map((stat) => {
        ...
        let required_counters = []
          ...
          .map((m: string) => m.split('-').reverse()[0]);
        required_counters = required_counters.filter((counter) => counter.indexOf('*') < 0);
        const counters_exist =
          required_counters.map((counter: any) => teams_counters.indexOf(counter) < 0).filter((f) => f).length < 1;
